# medigo-extractors-v2 worker-5 (batch 1)

你的工作目录: `/home/sophomores/code/medigo-w5` (git branch `work/v2-batch1-w5`).
任务清单: shanxiang, sanjieke, lexueyun
...
### 1. shanxiang (csslcloud)

源码目录: `~/code/xwz-downloader-source-release/decompiled_full/Mooc/Courses/Shanxiang/`
Go 目标文件: `internal/extractor/shanxiang/shanxiang.go`
...
### 2. sanjieke (single-api)

源码目录: `~/code/xwz-downloader-source-release/decompiled_full/Mooc/Courses/Sanjieke/`
Go 目标文件: `internal/extractor/sanjieke/sanjieke.go`
当前状态: STUB (verify_full_alignment.py 报)

预估工作量: 30-60 分钟
关键提示: 看源码 _get_infos / _get_video_url 主流程, 直接 HTTP+JSON parse 即可

### 3. lexueyun (single-api)

源码目录: `~/code/xwz-downloader-source-release/decompiled_full/Mooc/Courses/Lexueyun/`
Go 目标文件: `internal/extractor/lexueyun/lexueyun.go`
当前状态: STUB (verify_full_alignment.py 报)
...
# 然后报告: "worker-5 done, N/M sites PASS, branch pushed"
```
