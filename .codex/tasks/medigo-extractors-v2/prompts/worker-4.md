# medigo-extractors-v2 worker-4 (batch 1)

你的工作目录: `/home/sophomores/code/medigo-w4` (git branch `work/v2-batch1-w4`).
任务清单: qihang, nmkjxy, cnmooc
...
### 1. qihang (csslcloud+bokecc)

源码目录: `~/code/xwz-downloader-source-release/decompiled_full/Mooc/Courses/Qihang/`
Go 目标文件: `internal/extractor/qihang/qihang.go`
当前状态: STUB (verify_full_alignment.py 报)

预估工作量: 90-150 分钟 (csslcloud + bokecc 双平台)
关键提示: csslcloud 流程同上 + bokecc 部分调 shared.BokeCCResolve(vid, siteid) (qihang siteid='A183AC83A2983CCC')

### 2. nmkjxy (single-api)

源码目录: `~/code/xwz-downloader-source-release/decompiled_full/Mooc/Courses/Nmkjxy/`
Go 目标文件: `internal/extractor/nmkjxy/nmkjxy.go`
当前状态: STUB (verify_full_alignment.py 报)
...
源码目录: `~/code/xwz-downloader-source-release/decompiled_full/Mooc/Courses/Cnmooc/`
Go 目标文件: `internal/extractor/cnmooc/cnmooc.go`
当前状态: STUB (verify_full_alignment.py 报)
...
# 然后报告: "worker-4 done, N/M sites PASS, branch pushed"
```
